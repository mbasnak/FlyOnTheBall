...
for i = 1:length(smoothedSF)
    forwardVelocity{i} = smoothedSF{1,i}.xVel(1:751);
end

forwardVelocity = cell2mat(forwardVelocity);
forwardVelocity = reshape(forwardVelocity,[length(smoothedSF{1}.xVel),length(forwardVelocity)/length(smoothedSF{1}.xVel)]);
...
%(1) Stripe fixation trials
for i = 1:length(smoothedSF)
    angularVelocity{i} = smoothedSF{i}.angularVel(1:751);
end

angularVelocity = cell2mat(angularVelocity);
angularVelocity = reshape(angularVelocity,[length(smoothedSF{1}.angularVel),length(angularVelocity)/length(smoothedSF{1}.angularVel)]);
...
figure,
% scatter(posx{i},posy{i},0.5,timeTraj{i})
% colorbar
