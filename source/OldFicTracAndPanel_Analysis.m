...
save(strcat(path,'angVel', file(11:end-4), '.mat'),'angularVelocity');
...
saveas(gcf,strcat(path,'ActivityRP_ExpNum', file(11:end-4), '.svg'))
...
angPos = wrapTo360(posToDeg);

figure,
plot(time,angPos,'k','HandleVisibility','off')
...
%save(strcat(path,'angPos', file(11:end-4), '.mat'),'angPos');
...
figure,
plot(wrapTo180(fliplr(flyPosToDegMoving)),fliplr(time),'k','HandleVisibility','off')
xlabel('Heading angle (deg)'); ylabel('Time (s)');
title('Angular position of the Fly as a function of time');
xlim([-180 180]);
saveas(gcf,strcat(path,'AngulaPosFlyInTime180_ExpNum', file(11:end-4), '.svg'))
