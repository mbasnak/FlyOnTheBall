...
function ExpEmptyTrial2P(flyNum,expNum)
...
cd(strcat('Z:\Wilson Lab\Mel\Imaging\'));

daqreset %reset DAC object
devID = 'Dev3';  % Set device ID (to know what the ID is, you can type "daq.getDevices"
...
aI = niOI.addAnalogInputChannel( devID , 1:3 , 'Voltage' );
...
if expNum == 1 %if it's the first experiment for this fly
   cd(strcat('Z:\Wilson Lab\Mel\Imaging\',date)); %move to today's folder
   mkdir (strcat('flyNum',num2str(flyNum))) %inside that folder make a folder for this fly
   cd(strcat('Z:\Wilson Lab\Mel\Imaging\',date,'\flyNum',num2str(flyNum)));
   getFlyInfo() %get fly's details
else
   cd(strcat('Z:\Wilson Lab\Mel\Imaging\',date,'\flyNum',num2str(flyNum))); %otherwise move to this fly's folder
end
